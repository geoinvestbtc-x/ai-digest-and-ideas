...
def _call_llm(category, candidates, picks_n=5, attempt=0):
    ...
    print(f"[ai-digest][llm] cat={category} model={MODEL} candidates={len(candidates)} "
    ...
    print(f"[ai-digest][llm] cat={category} status={r.status_code} raw_len={len(r.text or '')}")
    ...
    try:
        ...
        print(f"[ai-digest][llm] ⚠ structure error: {e}")
    ...
    print(f"[ai-digest][llm] cat={category} finish={finish_reason} "
    ...
    if finish_reason in ('length', 'max_output_tokens'):
        print(f"[ai-digest][llm] ⚠ TRUNCATED! reasoning={reasoning_tokens}/{completion_tokens}")

    content = (content or "").strip()

    if not content:
        msg_obj = data_json.get('choices', [{}])[0].get('message', {})
        print(f"[ai-digest][llm] ⚠ EMPTY content! keys={list(msg_obj.keys())}")
    ...
    for category, arr in by.items():
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if not raw:
                    print(f"[ai-digest][llm] ⚠ empty for {category} (attempt {attempt})")
                ...
                print(f"[ai-digest][llm] ✅ cat={category} picks={len(out_picks)} "
                ...
                print(f"[ai-digest][llm] ❌ {category} attempt={attempt}: {e}")
                print(f"[ai-digest][llm]   snippet: {snippet}")
                if attempt < max_attempts - 1:
                    time.sleep(2)

        if last_error is not None:
            print(f"[ai-digest][llm] ⚠ fallback for {category}")
