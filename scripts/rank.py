...
def run(items, max_candidates_per_category=25):
    ...
    for cat in sorted(all_cats):
        ...
        print(
            f"[ai-digest][rank] cat={cat} in={total_in} passed={passed} "
            f"rejected={rejected}"
            + (f" reasons: {reason_str}" if reason_str else "")
        )
        for ex in rejected_examples.get(cat, []):
            print(f"[ai-digest][rank]   example: {ex}")
        ...
        if cc and cc['velocity']:
            ...
            print(f"[ai-digest][rank] cat={cat} avg_velocity={avg_v:.2f} "
    ...
    for cat, arr in by_cat.items():
        ...
        if top:
            ...
            print(f"[ai-digest][rank] cat={cat} top_scores={scores}")
        out.extend(top)
